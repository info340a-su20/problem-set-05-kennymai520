...
img.setAttribute('alt', 'A beautiful rainbow');
...
function createColorBox(color, pxSize) {
  let newD = document.createElement('div');
  newD.classList.add('d-inline-block');
  newD.style.backgroundColor = color;
  newD.style.width = pxSize + 'px';
  newD.style.height = pxSize + 'px';
  return newD;
}
...
function getElementWidth(inputElement) {
  let element = inputElement.getBoundingClientRect();
...
function renderPaletteRow(colorArray, parent) {
  ...
  let boxSize = getElementWidth(parent) / colorArray.size;
  for (let i = 0; i < colorArray.length; i++) {
    let box = createColorBox(colorArray[i], boxSize);
...
function renderPaletteTable() {
  ...
  for (const property in COLORS_9) {
    renderPaletteRow(COLORS_9[property], parent);
  }
}
...
let element = document.querySelector('header p');
